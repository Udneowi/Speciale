...
class Net(nn.Module):
    ...
    def __init__(self, embedding_dim, hidden_dim, n_loc_rank, n_loc_type, n_layers):
        ...
        self.weight_dist = nn.Parameter(torch.Tensor([1]).float(),requires_grad=True).cuda()
        ...
        self.embeddings_global  = nn.Embedding(34279, embedding_dim)
    ...
    def train(self, data, data_test=None,early_stop = False,data_loc=None):
        for epoch in range(self.epochs):  
            ...
            for step, (inp, target, target_expl) in enumerate(self.batches(data)):
                ...
                tmp[0][target[0]>self.cut_off]=self.cut_off                            # Setting all location bigger than 20 to 20
                logits = torch.Tensor(inp.shape[1],self.bs,self.cut_off).cuda()   # Initialize logits tensor
                
                # Create other possible targets for every given location to compute logits for them
                for i in range(self.cut_off):
                    tmp[-1] = (tmp[-1]+1)%self.cut_off+(tmp[-1]//self.cut_off)*self.cut_off  # Add 1 to the type data and keeps it in the same same interval
                    tmp_20 = tmp[0]==self.cut_off                        # Remembering which locations are already 20 for the rank data 
                    tmp[0] = (tmp[0]+1)%self.cut_off                     # Add 1 to the rank data mod 20
                    tmp[0][tmp_20] = self.cut_off                        # Setting those who were already 20 to 20 again
                    logits[:,:,i] = torch.sum((out*self.model.embeddings_out(tmp.clone())),dim=2)  # Compute the logits of the output
                    
                logits_dist = torch.Tensor(inp.shape[1],self.bs,self.cut_off).cuda()   # Initialize logits tensor
...
# #                             logits_dist[ii,jj,kk] = distance(inp[-3:-1,ii,jj],data_loc[int(inp[5,ii,jj]),kk]).km
                ...
                target[0][target[0]>self.cut_off] = self.cut_off # Changing all target locations above 20 to 20.
                
                # Comuting the weighted loss between exploration and next location prediction
                loss = F.cross_entropy(logits.reshape(-1,self.cut_off), target[0].reshape(-1),ignore_index = self.cut_off) #*self.model.weight_loss_w2v + \
            ...
            with torch.no_grad(): # No grad since it is testing
                ...
                for pep in data.keys():
                    ...
                    for step, (inp, target, target_expl) in enumerate(self.batches(data[pep]["test"],train="test")):
                        ...
                        tmp = target.clone()
                        tmp[0][target[0]>self.cut_off]=self.cut_off
                        logits = torch.Tensor(inp.shape[1],1,self.cut_off).cuda()
                        for i in range(self.cut_off):
                            tmp[-1] = (tmp[-1]+1)%self.cut_off+(tmp[-1]//self.cut_off)*self.cut_off
                            tmp_20 = tmp[0]==self.cut_off
                            tmp[0] = (tmp[0]+1)%self.cut_off
                            tmp[0][tmp_20] = self.cut_off
                        ...
                        loss_vali += F.cross_entropy(logits.reshape(-1,self.cut_off), target[0].reshape(-1),ignore_index = self.cut_off)
                        guess_print = []
