...
class Net(nn.Module):
    ...
    def __init__(self, embedding_dim, hidden_dim, n_loc_rank, n_loc_type, n_layers):
        ...
        self.weight_loss_w2v = Variable(torch.Tensor([1]).float()).cuda()
        self.weight_expl = Variable(torch.Tensor([1]).float()).cuda()
...
class Brain():
    ...
    def train(self, data, data_test=None,early_stop = False):
